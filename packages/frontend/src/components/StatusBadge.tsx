...
export default function StatusBadge({ status, size = 'md' }: StatusBadgeProps) {
  ...
  const sizeClasses = size === 'sm' ? 'px-2 py-0.5 text-[10px]' : 'px-3 py-1.5 text-[13px]';

  return (
    <span
      className={`inline-flex items-center gap-1.5 ${sizeClasses} ${config.bg} ${config.text} font-medium rounded-full border border-bdr bg-surface-subtle`}
    >
